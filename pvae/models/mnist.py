...
class Mnist(VAE):
    def __init__(self, params):
        ...
        self._pz_logvar = nn.Parameter(
            torch.zeros(1, 1), requires_grad=params.learn_prior_std
        )
        self.modelName = "Mnist"

    def init_last_layer_bias(self, train_loader):
        if not hasattr(self.dec.dec.fc31, "bias"):
            return
        with torch.no_grad():
    ...
    def pz_params(self):
        return (
            self._pz_mu.mul(1),
            F.softplus(self._pz_logvar).div(math.log(2)).mul(self.prior_std),
            self.manifold,
        )
